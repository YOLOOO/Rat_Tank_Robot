...
class HardwareServo:
    def __init__(self):
        ...
        if self.pcb_version == 1:
            self.pwm_gpio12 = HardwarePWM(pwm_channel=0, hz=SERVO_PWM_FREQ, chip=0)  # Initialize PWM for GPIO 12 on chip 0
            self.pwm_gpio13 = HardwarePWM(pwm_channel=1, hz=SERVO_PWM_FREQ, chip=0)  # Initialize PWM for GPIO 13 on chip 0
        elif self.pcb_version == 2:
            self.pwm_gpio12 = HardwarePWM(pwm_channel=0, hz=SERVO_PWM_FREQ, chip=0)  # Initialize PWM for GPIO 12 on chip 2
